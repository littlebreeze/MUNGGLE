...
export default function DirectMessage (props) {
  ...
  const searchUserAPI = async (keyword) => {
    try {
      const response = await fetch(`https://i10a410.p.ssafy.io:443/search/user?keyword=${keyword}`);
  ...
  useEffect(() => {
    const fetchChatRoomData = async () => {
      try {
        // GET 요청 보내기
        const response = await axios.get("https://i10a410.p.ssafy.io:443/message");
