...
export default function RecordOnly2() {


    const { kakao } = window;
    const [lat, setLat] = useState(33.450701);
    const [lng, setLng] = useState(126.570667);
    const latRef = useRef(33.450701);
    const lngRef = useRef(126.570667);
    ...
    useEffect(() => {
        ...
        const options = {
            //유저 위치
            center: new kakao.maps.LatLng(latRef.current,latRef.current),
            level: 4
            };
        ...
        if(navigator.geolocation){
          navigator.geolocation.getCurrentPosition(function(position){
            ...
            locPosition.current = new kakao.maps.LatLng(latRef.current, lngRef.current);
            message.current = '<div style="padding:5px;">현재 위치</div>';
            displayMarker(locPosition.current, message.current);
          });
        } else {
          locPosition.current = new kakao.maps.LatLng(33.450701, 126.570667);
          message.current = 'geolocation 사용 불가';
        }
