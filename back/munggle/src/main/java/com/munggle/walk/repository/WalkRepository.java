...
public interface WalkRepository extends JpaRepository<Walk, Long> {
    ...
    // 회원의 전체 산책 목록
    Optional<List<Walk>> findAllByUserIdAndIsDeletedFalseAndIsPrivatedFalse(Long userId);

    // 산책 전체 목록
    Optional<List<Walk>> findAllByIsDeletedFalseAndIsPrivatedFalse();

    // 산책 월별 목록
    Optional<List<Walk>> findAllByIsDeletedFalseAndCreatedAtBetween(LocalDateTime start, LocalDateTime end);
